...
class NonVolatileMemDiode_V1:
    ...
    def A(self, x):
        return self.alphamax * x + self.alphamin * (1 - x)
    ...
    def update_H(self, Vpn):
        if Vpn >0:
            new_h = self.H + (1-self.H)/self.Stau(Vpn)
        else:
            new_h = self.H - self.H/self.Rtau(Vpn) 
